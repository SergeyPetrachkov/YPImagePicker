...
open class YPPickerVC: YPBottomPager, YPBottomPagerDelegate {
  ...
  weak var output: YPPickerVCDelegate?
  ...
  open override func viewDidLoad() {
    ...
    if YPConfig.screens.contains(.photo) {
      ...
      cameraVC?.didCapturePhoto = { [weak self] img in
        self?.output?.didSelectItems(items: [YPMediaItem.photo(p: YPMediaPhoto(image: img,
                                                                               fromCamera: true))])
      }
    }
    ...
    if YPConfig.screens.contains(.video) {
      ...
      videoVC?.didCaptureVideo = { [weak self] videoURL in
        self?.output?.didSelectItems(items: [YPMediaItem
  ...
  func close() {
    ...
    // this fucking thing ruins everything, race condition or deadlock or something like that
  ...
  func done() {
    ...
    if mode == .library {
      libraryVC.doAfterPermissionCheck { [weak self] in
        libraryVC.selectedMediaNewWay(photoCallback: { photo in
          self?.output?.didSelectItems(items: [YPMediaItem.photo(p: photo)])
        }, videoCallback: { video in
          self?.output?.didSelectItems(items: [YPMediaItem
                                                .video(v: video)])
        }, errorCallback: {
          self?.output?.didFailToExportItems()
        }, multipleItemsCallback: { items in
          self?.output?.didSelectItems(items: items)
        })
      }
    }
  }
